...
def search_max(board):
    current_max = (-1, -1), float("-inf") # current_max[0] is the coords, current_max[1] is the point value (-1,000,000 is smaller than all possible scores in score(board) and so will always be overwritten when possible
...
# Testing 2 bots against each other
def search_max_white(board):
    current_max = (-1, -1), -100000
    for row_i in range(len(board[0])):
        for col_i in range(len(board)):
            if board[row_i][col_i] == " ":
                prev_stone = board[row_i][col_i]
                board[row_i][col_i] = "w"
                curr_score = white_score(board)
                board[row_i][col_i] = " "
                if curr_score > current_max[1]:
                    current_max = (row_i, col_i), curr_score
    return current_max[0]
...
# For writing to a text file to easily view results
...
def play_gomoku_bots(board_size):
    ''' Simulates 2 gomoku bots playing against each other '''
    board = make_empty_board(board_size)
    board_height = len(board)
    board_width = len(board[0])
    
    board[board_height // 2][board_width // 2: board_width // 2 + 1] = ["b", "b"]
    board[board_height // 2 + 1][board_width //2 : board_width // 2 + 1] = ["w", "w"] 
    ...
    while True:
        ...
        f.write("Black move: (%d, %d)\n" % (move_y, move_x))
        board[move_y][move_x] = "b"
        f.write(return_board(board) + "\n")
        ...
        move_y, move_x = search_max_white(board)
        f.write("White move: (%d, %d)\n" % (move_y, move_x))
        board[move_y][move_x] = "w"
        f.write(return_board(board) + "\n")
...
# (0, 0)
    ...
    print("White's best move:", search_max_white(board))
